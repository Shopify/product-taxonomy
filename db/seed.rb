...
module DB
  class Seed
    ...
    def values_from(data)
      ...
      PropertyValue.insert_all(Data2::PropertyValueSerializer.unpack_all(data))
    ...
    def attributes_from(data)
      ...
      Property.insert_all(Data2::PropertySerializer.unpack_all(data["base_attributes"]))
      PropertiesPropertyValue.insert_all!(Data2::PropertiesPropertyValueSerializer.unpack_all(data["base_attributes"]))

      inserted_properties = Property.insert_all(
        Data2::ExtendedPropertySerializer.unpack_all(data["extended_attributes"]),
        returning: ["id", "base_friendly_id"],
      )
      PropertiesPropertyValue.insert_all!(
        Data2::ExtendedPropertiesPropertyValueSerializer.unpack_all(inserted_properties),
      )
    ...
    def categories_from(data)
      ...
      data.each do |vertical_data|
        ...
        Category.insert_all(Data2::CategorySerializer.unpack_all(vertical_data))
        CategoriesProperty.insert_all(Data2::CategoriesPropertySerializer.unpack_all(vertical_data))
      end
    ...
    def mapping_rules_from(data)
      ...
      data.each do |file|
        ...
        rules.each do |rule|
          input_product_hash = Data2::ProductSerializer.unpack(rule["input"].merge("type" => input_type))
          input_product = Product.find_or_create_by!(type: input_type, payload: input_product_hash)
          output_product_hash = Data2::ProductSerializer.unpack(rule["output"].merge("type" => output_type))
