...
class AllDataFilesImportTest < ActiveSupport::TestCase
  ...
  test "AttributeValues are consistent with values.yml" do
    @raw_values_data.each do |raw_value|
      deserialized_value = PropertyValue.new(Data2::PropertyValueSerializer.unpack(raw_value))
  ...
  test "Attributes are consistent with attributes.yml" do
    ...
    base_attributes.each do |raw_attribute|
      deserialized_attribute = Property.new(Data2::PropertySerializer.unpack(raw_attribute))
    ...
    extended_attributes.each do |raw_attribute|
      deserialized_attribute = Property.new(Data2::ExtendedPropertySerializer.unpack(raw_attribute))
  ...
  test "Categories are consistent with categories/*.yml" do
    @raw_verticals_data.flatten.each do |raw_category|
      deserialized_category = Category.new(Data2::CategorySerializer.unpack(raw_category))
    ...
    @raw_mapping_rules_data.each do |raw|
      ...
      raw[:content].fetch("rules").each do |raw_rule|
        deserialized_input_product = Product.new(
          payload: Data2::ProductSerializer.unpack(raw_rule["input"].merge("type" => input_type)),
          type: input_type,
        )
        deserialized_output_product = Product.new(
          payload: Data2::ProductSerializer.unpack(raw_rule["output"].merge("type" => output_type)),
          type: output_type,
        )
